...
public class TestShortestPath {
  ...
  private static int minDist(DistanceGraph g, int x, int y) {
    // This is slow, but that doesn't really matter for our small tests.
    int min = -1;
    for (Edge e : g.edges(x)) {
      if (e.destination == y && (min == -1 || e.distance < min)) {
        min = e.distance;
      }
    }
    assertWithMessage("the path contains a missing edge %s->%s", x, y)
        .that(min).isNotEqualTo(-1);
    return min;
  }
}
